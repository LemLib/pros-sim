...
void vexDeviceMotorPwmSet(V5_DeviceT device, __attribute((unused)) int32_t) {
    if (!device->exists || device->type != kDeviceTypeMotorSensor) {
        errno = ENODEV;
        return;
    }
...
void vexDeviceMotorServoTargetSet(V5_DeviceT device, __attribute((unused)) double) {
    if (!device->exists || device->type != kDeviceTypeMotorSensor) {
        errno = ENODEV;
        return;
    }
...
void vexDeviceMotorExternalProfileSet(V5_DeviceT device, __attribute((unused)) double, __attribute((unused)) int32_t) {
    if (!device->exists || device->type != kDeviceTypeMotorSensor) {
        errno = ENODEV;
        return;
    }
