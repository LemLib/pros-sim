...
std::vector<double> MotorGroup::get_actual_velocity_all(void) const {
    ...
    for (auto it = _ports.begin(); it < _ports.end(); it++) { return_vector.push_back(motor_get_actual_velocity(*it)); }
...
MotorBrake MotorGroup::get_brake_mode(const std::uint8_t index) const {
	...
	return static_cast<MotorBrake>(motor_get_brake_mode(_ports[index]));
}

std::vector<MotorBrake> MotorGroup::get_brake_mode_all(void) const {
	...
	for (auto it = _ports.begin(); it < _ports.end(); it++) {
		return_vector.push_back(static_cast<MotorBrake>(motor_get_brake_mode(*it)));
	}
	return return_vector;
}
...
std::vector<double> MotorGroup::get_efficiency_all(void) const {
    std::vector<double> return_vector;
    empty_MotorGroup_check_vector(PROS_ERR_F, return_vector);
    for (auto it = _ports.begin(); it < _ports.end(); it++) { return_vector.push_back(motor_get_efficiency(*it)); }
    return return_vector;
}
...
MotorUnits MotorGroup::get_encoder_units(const std::uint8_t index) const {
	...
	return static_cast<MotorUnits>(motor_get_encoder_units(_ports[index]));
}

std::vector<MotorUnits> MotorGroup::get_encoder_units_all(void) const {
	std::vector<MotorUnits> return_vector;
	empty_MotorGroup_check_vector(pros::v5::MotorUnits::invalid, return_vector);
	for (auto it = _ports.begin(); it < _ports.end(); it++) {
		return_vector.push_back(static_cast<MotorUnits>(motor_get_encoder_units(*it)));
	}
	return return_vector;
}
...
std::vector<std::uint32_t> MotorGroup::get_flags_all(void) const {
    std::vector<std::uint32_t> return_vector;
    empty_MotorGroup_check_vector(PROS_ERR, return_vector);
    for (auto it = _ports.begin(); it < _ports.end(); it++) { return_vector.push_back(motor_get_flags(*it)); }
    return return_vector;
}
...
MotorGears MotorGroup::get_gearing(const std::uint8_t index) const {
	...
	return static_cast<MotorGears>(motor_get_gearing(_ports[index]));
}

std::vector<MotorGears> MotorGroup::get_gearing_all(void) const {
	std::vector<MotorGears> return_vector;
	empty_MotorGroup_check_vector(pros::v5::MotorGears::invalid, return_vector);
	for (auto it = _ports.begin(); it < _ports.end(); it++) {
		return_vector.push_back(static_cast<MotorGears>(motor_get_gearing(*it)));
	}
	return return_vector;
}
...
std::int32_t MotorGroup::set_brake_mode(const motor_brake_mode_e_t mode, const std::uint8_t index) const {
	empty_MotorGroup_check(PROS_ERR);
	MotorGroup_index_check(PROS_ERR, index);
	return motor_set_brake_mode(_ports[index], mode);
}

std::int32_t MotorGroup::set_brake_mode(const MotorBrake mode, const std::uint8_t index) const {
	empty_MotorGroup_check(PROS_ERR);
	MotorGroup_index_check(PROS_ERR, index);

	return motor_set_brake_mode(_ports[index], static_cast<motor_brake_mode_e_t>(mode));
}

std::int32_t MotorGroup::set_brake_mode_all(const motor_brake_mode_e_t mode) const {
	empty_MotorGroup_check(PROS_ERR);
	for (auto it = _ports.begin() + 1; it < _ports.end(); it++) {
...
std::int32_t MotorGroup::set_brake_mode_all(const MotorBrake mode) const {
	empty_MotorGroup_check(PROS_ERR);
	for (auto it = _ports.begin() + 1; it < _ports.end(); it++) {
		motor_set_brake_mode(*it, static_cast<motor_brake_mode_e_t>(mode));
	}
	return motor_set_brake_mode(_ports[0], static_cast<motor_brake_mode_e_t>(mode));
}
...
std::int32_t MotorGroup::set_encoder_units_all(const motor_encoder_units_e_t units) const {
	empty_MotorGroup_check(PROS_ERR);
	for (auto it = _ports.begin() + 1; it < _ports.end(); it++) {
...
std::int32_t MotorGroup::set_encoder_units(const motor_encoder_units_e_t units, const std::uint8_t index) const {
	empty_MotorGroup_check(PROS_ERR);
	MotorGroup_index_check(PROS_ERR, index);
	return motor_set_encoder_units(_ports[index], units);
}

std::int32_t MotorGroup::set_encoder_units(const MotorUnits units, const std::uint8_t index) const {
	empty_MotorGroup_check(PROS_ERR);
	MotorGroup_index_check(PROS_ERR, index);
...
std::int32_t MotorGroup::set_gearing(const MotorGear gearset, const std::uint8_t index) const {
	empty_MotorGroup_check(PROS_ERR);
	MotorGroup_index_check(PROS_ERR, index);
...
std::int32_t MotorGroup::set_gearing_all(const MotorGear gearset) const {
	empty_MotorGroup_check(PROS_ERR);
	for (auto it = _ports.begin() + 1; it < _ports.end(); it++) {
